...
$dataSetPath = "C:\Data"      # Path to the directory containing files
...
function Read-Files {
    ...
    while ((Get-Date) -lt $endActionTime) {
        ...
        if ($files.Count -gt 0) {
            ...
            try {
                # Read the content of the file (important: handle file locking)
                $content = Get-Content -Path $filePath -ErrorAction SilentlyContinue
                if ($content)
                {
                    # Do nothing with the content, just read it.
                }
                # Log
                Log-DataToCSV -CPUUsage (Get-CpuUsage) -MemoryUsage (Get-MemoryUsage) -ReadIOPS (Get-DiskIOPS).ReadIOPS -WriteIOPS (Get-DiskIOPS).WriteIOPS -Action "Read File: $($randomFile.Name)"
                Write-Host "Read file: $($randomFile.Name)" -ForegroundColor Cyan
            } catch {
                #error reading.
                Log-DataToCSV -CPUUsage (Get-CpuUsage) -MemoryUsage (Get-MemoryUsage) -ReadIOPS (Get-DiskIOPS).ReadIOPS -WriteIOPS (Get-DiskIOPS).WriteIOPS -Action "Error Reading File: $($randomFile.Name)"
                Write-Host "Error reading file: $($randomFile.Name)" -ForegroundColor Red
            }
