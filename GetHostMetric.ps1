...
function Get-Entropy {
    ...
    if (Test-Path $entPath) {
        try {
            ...
            # Find first matching line and extract value
            foreach ($line in $output) {
                if ($line -match "Entropy\s+=\s+([0-9\.]+)") {
                    return [Math]::Round([double]$matches[1], 3)
                }
            }

            # If no match found
            Write-Warning "Entropy not found in ent.exe output for $filePath"
            return -1
        } catch {
            Write-Warning "ent.exe failed for $filePath"
            return -1
        }
    }
}
